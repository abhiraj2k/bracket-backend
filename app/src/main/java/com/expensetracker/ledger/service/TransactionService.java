package com.expensetracker.ledger.service;

import com.expensetracker.budget.dto.ExpenseEventRequest;
import com.expensetracker.budget.dto.LineItemExpense;
import com.expensetracker.budget.service.BudgetPeriodService;
...
import com.expensetracker.ledger.dto.*;
import com.expensetracker.ledger.entity.TransactionHeader;
import com.expensetracker.ledger.entity.TransactionLineItem;
import com.expensetracker.ledger.repository.TransactionHeaderRepository;
import com.expensetracker.ledger.repository.TransactionLineItemRepository;
...
public class TransactionService {
    ...
    public TransactionResponse create(UUID userId, CreateTransactionRequest request) {
        ...
        // Truly atomic: balance update and ledger write share the same @Transactional
        adjustBalance(saved.getSourceAccountId(), saved.getDestAccountId(),
                saved.getTransactionType(), saved.getTotalAmount());
        ...
        if (request.getTransactionType() == TransactionType.EXPENSE) {
            List<LineItemExpense> lineItemExpenses = saved.getLineItems().stream()
                    ...
                    .toList();
            ExpenseEventRequest event = new ExpenseEventRequest();
            event.setUserId(userId);
            event.setTransactionDate(saved.getTransactionDate());
            event.setLineItems(lineItemExpenses);
    ...
    public void delete(UUID id, UUID userId) {
        TransactionHeader header = headerRepo.findByIdAndUserId(id, userId)
                .orElseThrow(() -> new ResourceNotFoundException("Transaction", id));
        // DB delete first — if balance call below throws, @Transactional rolls back everything
        headerRepo.delete(header);
        reverseBalance(header.getSourceAccountId(), header.getDestAccountId(),
                header.getTransactionType(), header.getTotalAmount());
    }

    private void adjustBalance(UUID sourceId, UUID destId, TransactionType type, BigDecimal amount) {
        BalanceAdjustRequest req = new BalanceAdjustRequest();
        req.setSourceAccountId(sourceId);
        req.setDestAccountId(destId);
        req.setTransactionType(type);
        req.setAmount(amount);
        accountService.adjustBalance(req);
    }

    private void reverseBalance(UUID sourceId, UUID destId, TransactionType type, BigDecimal amount) {
        BalanceAdjustRequest req = new BalanceAdjustRequest();
        req.setAmount(amount);
        if (type == TransactionType.TRANSFER) {
            req.setSourceAccountId(destId);   // swap: debit the original dest
            req.setDestAccountId(sourceId);   //       credit the original source
            req.setTransactionType(TransactionType.TRANSFER);
        } else {
            req.setSourceAccountId(sourceId);
            req.setTransactionType(type == TransactionType.EXPENSE
